...
export class LunoService implements ICollectionService {
    ...
    async getBalances(): Promise<Wallet[]> {
        try {
            const response = await request.get(URL_BALANCE).auth(this.config.luno.API_KEY_ID, this.config.luno.API_KEY_SECRET);
            return (JSON.parse(response).balance as LunoBalance[]).map(
                bal => new Wallet('Luno', +bal.balance, bal.asset.toLowerCase(), new Date())
            );
        } catch (err) {
            console.error(err);
            return null;
    ...
    async getTicker(ticker: string): Promise<Ticker> {
        try {
            const response = JSON.parse(
                await request.get(URL_TICKER + ticker).auth(this.config.luno.API_KEY_ID, this.config.luno.API_KEY_SECRET)
            ) as LunoTicker;
